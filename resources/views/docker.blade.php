...
<head>
    <meta charset="utf-8">
    <title># Docker 搭建 Nginx + php-fpm + mysql</title>
    <style>
...
*/
    ...
    <h1 id="toc_0">Docker 搭建 Nginx + php-fpm + mysql</h1>

    <blockquote>
        <p>Author：邹智<br/>
            PS: 我自己的所有配置文件可以去我的 <a href="https://github.com/jacks121/docker">Github</a> 中Clone。<br/>
            全文中加粗部分，是我自己踩过的坑。希望对大家有帮助。</p>
    </blockquote>

    <h2 id="toc_1">创建目录</h2>

    <ol>
        <li>创建项目目录 <code>mkdir project</code></li>
        <li>进入project目录 <code>cd project</code> </li>
        <li>创建Nginx目录 <code>mkdir nginx</code></li>
        <li>创建php-fpm目录 <code>mkdir php-fpm</code></li>
        <li>创建mysql文件存储目录 <code>mkdir data</code></li>
    </ol>

    <h2 id="toc_2">配置Nginx</h2>

    <ul>
        <li>创建Dockerfile文件</li>
        <li><code>fastcgi_pass my-app:9000;</code> <strong>my-app是在docker-compose.yml中设定的services(php-fpm)名称</strong></li>
    </ul>
...
http
{
    ...
    <h2 id="toc_3">配置php-fpm</h2>

    <ul>
        <li>去 <a href="https://hub.docker.com/_/php">Dockerhub官方php镜像</a> 选择你需要的php-fpm版本，并且下载相关的所有文件。通常包括Dcokerfile、docker-php-ext-configure、docker-php-ext-install、docker-php-entrypoint、docker-php-ext-enable、docker-php-source 这六个文件。</li>
        <li>将这些文件放入事先创建好的php-fpm文件夹中</li>
        <li><strong>如果需要安装扩展，可以通过官方的方式。但我并不推荐，因为要重新build整个镜像。此过程中，切不论“墙”的因素，各种报错就够喝一壶了。</strong></li>
    </ul>
    ...
    <ul>
        <li><strong>下面推荐另一种安装扩展的方式</strong></li>
    </ul>

    <p>首先进入容器中，然后 <code>cd /usr/local/bin</code> 找到其中的 <code>docker-php-ext-install</code> 命令直接安装。例如：<code>docker-php-ext-install pdo pdo_mysql</code>。</p>

    <h2 id="toc_4">编写docker-compose.yml文件</h2>

    <ul>
        <li><strong>建议配置好一个服务先测试是否有问题，再此基础上追加新的服务，这样就算有问题了，也比较容易定位。经验之谈。</strong></li>
    </ul>
...
services:
  ...
  db:
    ...
    <h2 id="toc_5">启动并使用docker-compose</h2>

    <ul>
        <li>进入docker-compose.yml文件所在的文件夹 <code>cd project</code></li>
        <li>创建并启动所有容器 <code>docker-compose up -d</code> -d是指在后台运行 <strong>(如果容器配置有所变动，在此之前先执行 <code>dcoker-compose build</code>)</strong></li>
        <li>进入容器 <code>docker-compose exec $serivce_name bash</code> 其中 <code>$service_name</code> 是你docker-compose.yml文件中定义的services名称，并不是 <code>docker-compose ps</code> 查看到的名称。</li>
        <li>停止/启动 <code>docker-compose stop / docker-compose start</code></li>
        <li>停止并删除容器 <code>docker-compose down</code></li>
        <li>水平扩展/负载均衡 <code>docker-compose up --scale my-app=3</code> 可以通过这种方式同时开启多个服务 配置也需要做相应的改动</li>
    </ul>
